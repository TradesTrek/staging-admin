...
export default function AddSubscription({ addUserCloseModal }) {
  ...
  const validate = (values) => {
    ...
    if (values.packageAmount<100) {
      errors.packageAmount = 'Package Amount must be greater then or equal to 100';
    }
          ...
          <form id="addLocks" onSubmit={handleSubmit} noValidate>
            ...
            <div className="form--item">
              ...
              >
                ...
                <option
                  value='hourly'
                  selected={formValues.packageDuration == 'hourly' ? true : false}
                >
                  Hourly
                </option>
                <option
                  value='monthly'
                  selected={formValues.packageDuration == 'monthly' ? true : false}
                >
                  Monthly
                </option>
                <option
                  value='quarterly'
                  selected={formValues.packageDuration == 'quarterly' ? true : false}
                >
                  Quarterly
                </option>
                <option
                  value='annually'
                  selected={formValues.packageDuration == 'annually' ? true : false}
                >
                  Annually
                </option>
                <option
                  value='free-lifetime'
                  selected={formValues.packageDuration == 'free-lifetime' ? true : false}
                >
                  free-lifetime
                </option>
              </select>
              <div className="invalid-feedback">{formErrors?.packageDuration}</div>
            </div>
            <div className="form--item">
              <label className="form--label" htmlFor="lockModel">
                Package Amount*
