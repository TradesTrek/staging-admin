...
import AddHoliday from "../../components/subsector/AddSubSector";
...
export default function AllUsers() {
 ...
 useEffect(() => {
 getExchanges(search,option,currentPage);
 }, [search,option]);
 const getExchanges = (search,data,page) => {
 setIsLoading(true)
 stockService
 .getSubSectors(search,data,page)
 .then((res) => {
 if (res.success) {
 setHoliday(res.data);
 }
 ...
 let response = await stockService.deleteSubSector(userId);
 if (response.success) {
 ...
 <h1 className="dashboard__title">Stock Sub Sectors</h1>
 ...
 <td onClick={()=>setTableAction(false)}>{item.name}</td>
